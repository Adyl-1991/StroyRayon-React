...
export function addCartItem(items, product, quantity = 1, variant = null) {
  ...
  return [
    ...items,
    {
      ...
      titleKg: product.titleKg || product.name,
      titleRu: product.titleRu,
      price: Number(variant?.price ?? product.price ?? 0),
      image: getProductImage(product, variant || 'main'),
      unit: variant?.unit || product.unit,
      unitKg: variant?.unit || product.unit,
      quantity: safeQuantity,
      sku: variant?.sku || product.sku,
      variantId: variant?.id,
      variantSize: variant?.size || variant?.titleKg,
      variantTitleKg: variant?.titleKg || variant?.size,
      variantTitleRu: variant?.titleRu || variant?.size,
      variantSku: variant?.sku,
      packageInfo: variant?.packageInfo || product.packageInfoKg || product.minOrder,
      packageInfoRu: variant?.packageInfoRu || product.packageInfoRu || product.packRu || product.minOrderRu,
    },
  ]
}
